...
def compute_loss(
    model: T5ForPretrain, codewords: Tensor, x: Tensor, labels: Tensor
) -> Tensor:
    """
    ...
    for i in range(vecid_len):
        ...
        if codewords is not None:
            # Standard PQ: append centroid to refine next prediction
            centroid = torch.index_select(
                codewords[i], dim=0, index=tokid
            )  # (batch_size, vec_dim / M)
            centroid = model.output_proj(centroid.to(x)).unsqueeze(
                dim=1
            )  # (batch_size, 1, vec_dim)
            x = torch.cat([x, centroid], dim=1)  # (batch_size, seq_len+1, vec_dim)
        # IVF-PQ: predict all tokens from query only (no token history)
...
def main(args):
    ...
    dataloader = DataLoader(
        dataset, batch_size=args.batch_size, shuffle=True, pin_memory=True, generator=g
    )
    codewords = dataset.codewords.to(device) if dataset.codewords is not None else None
