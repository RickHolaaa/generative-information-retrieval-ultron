...
class T5ForPretrain(T5ForConditionalGeneration):
    def __init__(self, config: T5Config, args):
        ...
        self.output_proj = nn.Sequential(
            nn.Linear(
                Sift1mDataset.VECTOR_DIM // args.num_subspace, self.config.d_model // 2
            ),
            nn.Linear(self.config.d_model // 2, self.config.d_model),
        )

        # For IVF-PQ, use full vector dimension in first layer
        if hasattr(args, "use_ivf_pq") and args.use_ivf_pq:
            self.output_proj = nn.Sequential(
                nn.Linear(Sift1mDataset.VECTOR_DIM, self.config.d_model // 2),
                nn.Linear(self.config.d_model // 2, self.config.d_model),
            )
    ...
    def forward(
        ...
        lm_logits = self.lm_head(
            sequence_output
        )  # (batch_size, output_len, num_clusters)
